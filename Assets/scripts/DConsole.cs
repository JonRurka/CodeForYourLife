...
public class DConsole : MonoBehaviour {
    ...
    public static string ConsoleTittle = "C4YL Debug Console";
    ...
    public static bool IsOpen {
        get { return DConsole.Instance._isOpen; }
        set { DConsole.Instance._isOpen = value; }
    }
    ...
    /// </summary>
    static DConsole Instance {
        get {
            if (_instance == null) {
                _instance = FindObjectOfType(typeof(DConsole)) as DConsole;
                ...
                _instance = console.AddComponent<DConsole>();
            }
    ...
    static DConsole _instance;
    Dictionary<string, CommandFunction> _cmdTable = new Dictionary<string, CommandFunction>();
    ...
    struct Message {
        ...
        void checkDebug(int level) {
            ...
            if (DConsole.logLevel >= level) {
                valid = true;
                if (DConsole.logOutput)
                    if (File.Exists(global::System.IO.Path.Combine(directory, DConsole.logName + ".log"))) {
                        using (StreamWriter sw = File.AppendText(global::System.IO.Path.Combine(directory, DConsole.logName + ".log"))) {
    ...
    public struct CommandDescription {
        ...
        public DConsole.CommandFunction callback;

        public CommandDescription(string _plugin, string _command, string _command_args, string _description_small, string _description_Long, DConsole.CommandFunction _callback) {
            plugin = _plugin;
        ...
        public CommandDescription(string _plugin, string _command, string _command_args, string _description_small, DConsole.CommandFunction _callback) {
            plugin = _plugin;
            command = _command.ToLower();
            command_args = _command_args;
            description_small = _description_small;
    ...
    void OnEnable() {
        ...
        RegisterCommand(new CommandDescription("Game", "close", string.Empty, "closes the console.", CMDClose));
        RegisterCommand(new CommandDescription("Game", "clear", string.Empty, "Clear the console.", CMDClear));
        RegisterCommand(new CommandDescription("Game", "sys", string.Empty, "display system info.", "display detailed info about the system.", CMDSystemInfo));
        RegisterCommand(new CommandDescription("Game", "help", "[command]", "displays this prompt", "Displays the help prompt. Enter in a command name for the second argument to get a detailed description of the command.", CMDHelp));
...
//#endif
        ...
        _windowRect = GUI.Window(-1111, _windowRect, LogWindow, string.Format("{0} v{1}\tfps: {2:00.0}", ConsoleTittle, "1.0", fps.current));
            ...
            if (evt.isKey && evt.type == EventType.KeyUp) {
                if (evt.keyCode == KeyCode.Return) {
                    try {
                        ...
                        DConsole.LogError(string.Format("message: {0}.\nSource {1}.\nTrace: {2}.", e.Message, e.Source, e.StackTrace));
                    }
                    _inputString = string.Empty;
                }
    ...
    public static object Log(object message) {
        DConsole.Instance.LogMessage(Message.Log(message));
    ...
    public static object Log(object message, MessageType messageType) {
        DConsole.Instance.LogMessage(new Message(message, messageType));
    ...
    public static object Log(object message, Color displayColor) {
        DConsole.Instance.LogMessage(new Message(message, displayColor));
    ...
    public static object Log(object message, MessageType messageType, Color displayColor) {
        DConsole.Instance.LogMessage(new Message(message, messageType, displayColor));
    ...
    public static object LogWarning(object message) {
        DConsole.Instance.LogMessage(Message.Warning(message));
    ...
    public static object LogError(object message) {
        DConsole.Instance.LogMessage(Message.Error(message));

        return message;
    }
    public static object LogSystem(object message) {
        DConsole.Instance.LogMessage(Message.System(message));

        return message;
    }
    public static object LogDebug(object message) {
        DConsole.Instance.LogMessage(Message.Developer(message));

        return message;
    }
    public static object LogInfo(object message) {
        DConsole.Instance.LogMessage(new Message(message, MessageType.INFO));
    ...
    public static string Execute(string commandString) {
        ...
        if (DConsole.Instance._cmdTable.ContainsKey(cmd)) {
            return DConsole.Instance._cmdTable[cmd](input.ToArray()).ToString();
        }
        else {
    ...
    public static string[] Commands() {
        string[] commands = new string[DConsole.Instance._cmdTable.Keys.Count];
        DConsole.Instance._cmdTable.Keys.CopyTo(commands, 0);
        return commands;
    }
    ...
    public static void RegisterCommand(CommandDescription command) {
        try {
            DConsole.Instance.RegisterCommandCallback(command, command.callback);
        }
        catch (Exception e) {
            DConsole.LogError(string.Format("Failed to add command \"{0}\": {1}", command.command, e.Message));
        }
    }
    ...
    public static void UnRegisterCommand(string commandString) {
        DConsole.Instance.UnRegisterCommandCallback(commandString);
    }
    #endregion
    ...
    object CMDHelp(params string[] args) {
        ...
        if (args.Length == 1) {
            ...
            if (_cmdTable.ContainsKey(args[1])) {
                output.AppendLine("Available commands (<> = required, [] = optional):");
                ...
                output.AppendLine("Command not found.");
        }
        else if (args.Length == 2 && args[1].ToLower().StartsWith("-p")) {
            output.AppendLine("Please specify a plugin.");
        }
        else if (args.Length > 3) {
            output.AppendLine("To many arguments.");
        }
        else
    ...
    //--- Local version. Use the static version above instead.
    void RegisterCommandCallback(CommandDescription command, CommandFunction commandCallback) {
        _cmdTable[command.command.ToLower()] = new CommandFunction(commandCallback);
